...
namespace ELibraryManagement
{
    public partial class adminbookinventory : System.Web.UI.Page
    {
        ...
        protected void Button3_Click(object sender, EventArgs e)
        {
            ...
            {
                Response.Write("<script>alert('Book allready Exist, try some Book ID');</script>");
            }
            else
            {
                addNewBooks();
        ...
        void updateBookByID()
        {
            if (checkIfBookExists())
            {
                try
                {
                    ...
                    SqlCommand cmd = new SqlCommand("UPDATE book_master_tb1 SET book_name=@book_name, genre=@genre, author_name=@author_name," +
        ...
        void getBookById()
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("SELECT * from book_master_tb1 WHERE book_id='"+ TextBox3.Text.Trim() +"';", con);
        ...
        void deleteBookByID()
        {
            if (checkIfBookExists())
            {
                try
                {
                    ...
                    SqlCommand cmd = new SqlCommand("DELETE from book_master_tb1 WHERE book_id='" + TextBox3.Text.Trim() + "'", con);
        ...
        bool checkIfBookExists()
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("SELECT * from book_master_tb1 where book_id='" + TextBox3.Text.Trim() + "' OR book_name='"+TextBox2.Text.Trim() +"';", con);
                SqlDataAdapter da = new SqlDataAdapter(cmd);
                DataTable dt = new DataTable();
                da.Fill(dt);
                if (dt.Rows.Count >= 1)
        ...
        void addNewBooks()
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("INSERT INTO book_master_tb1(book_id,book_name,genre,author_name,publisher_name,publish_date," +
                    "language,edition,book_cost,no_of_pages,book_description,actual_stock,current_stock,book_img_link) values(@book_id,@book_name," +
                    "@genre,@author_name,@publisher_name,@publish_date,@language,@edition,@book_cost,@no_of_pages,@book_description,@actual_stock," +
